...
**mcp-b24** — это AI-ассистент для вашего портала Битрикс24. Подключив его к AI-клиенту (Claude, Cursor, VS Code, Codex CLI), вы управляете CRM, задачами, чатами, файлами, календарём, почтой, телефониями и бизнес-процессами **обычными фразами**. Сервер сам выполняет реальные действия на портале.
