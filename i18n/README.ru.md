...
|-----------|------------|--------|
| `bx24_crm_leads` | Лиды: CRUD, контакты, товарные позиции, пользовательские поля, конвертация, конфиг карточки | CRM |
| `bx24_crm_deals` | Сделки + воронки, товарные позиции, привязки контактов, recurring, пользовательские поля | CRM |
| `bx24_crm_contacts` | Контакты: CRUD, привязки компаний, поля, конфиг карточки | CRM |
| `bx24_crm_companies` | Компании: CRUD, привязки контактов, поля, конфиг карточки | CRM |
| `bx24_crm_invoices` | Счета (SMART_INVOICE, entityTypeId=31) + стадии | CRM |
| `bx24_crm_products` | Торговый каталог: товары, разделы, цены, склады, типы цен, единицы, НДС, коэффициенты, складские документы, свойства, вариации/SKU/услуги | CRM |
| `bx24_crm_activities` | Дела (звонки/встречи/письма) + todo + configurable + типы + бейджи + полный таймлайн | CRM |
| `bx24_crm_requisites` | Реквизиты + пресеты + банковские реквизиты + привязки + пользовательские поля | CRM |
| `bx24_crm_duplicates` | Поиск и слияние дублей + справочник статусов + volatile types | CRM |
| `bx24_smart_processes` | Умные процессы: типы + элементы | CRM |
| `bx24_crm_quotes` | Коммерческие предложения: CRUD, товарные позиции, привязки контактов, поля | CRM |
| `bx24_crm_documents` | Генератор документов: шаблоны, документы, нумераторы, привязки, провайдеры | CRM |
| `bx24_crm_currency` | Валюты: CRUD, базовая валюта, локализации | CRM |
| `bx24_crm_webform` | Веб-формы + результаты + опции | CRM |
| `bx24_crm_tracking` | Отслеживание: источники, треки, каналы | CRM |
| `bx24_crm_automation` | Триггеры автоматизации CRM | CRM |
...
| `bx24_im` | Мессенджер: сообщения, уведомления, поиск, счётчики, отделы, события v2, файлы v2 | collab |
| `bx24_im_chat` | Чаты: создание, участники, owner/manager, mute, сообщения | collab |
| `bx24_conf` | Видеоконференции | collab |
| `bx24_calendar` | Календарь: события, разделы, встречи, ресурсы, доступность, настройки | collab |
| `bx24_openlines` | Открытые линии: конфиги, сессии, операторы, CRM-связи, сеть | collab |
| `bx24_bots` | Чат-боты v2: регистрация, чаты, сообщения, реакции, команды, файлы, события | collab |
| `bx24_users` | Пользователи: текущий, поиск, поля (вкл. list), CRUD | org |
| `bx24_departments` | Отделы / оргструктура | org |
| `bx24_time` | Учёт рабочего времени (timeman) + time control + сетевые диапазоны + графики + записи | org |
| `bx24_hr` | HR: сотрудники, приглашение, увольнение, перевод | org |
| `bx24_lists` | Универсальные списки (инфоблоки) + секции + типы полей | biz |
| `bx24_mail` | Почта: ящики, сообщения, отправка/ответ/пересылка, фильтры, сервисы, письмо→задача/событие/чат/CRM | biz |
| `bx24_reports` | Аналитика и отчёты | biz |
| `bx24_marketing` | Сегменты, рассылки, фильтры лидов | biz |
| `bx24_workflows` | Бизнес-процессы и роботы: шаблоны, экземпляры, задачи, CRUD роботов/активностей, события | biz |
| `bx24_telephony` | Телефония: внешние линии/звонки, SIP, voximplant (обратные/инфо-звонки, TTS, линии, статистика) | biz |
...
- **CRM** — лиды/контакты/компании/сделки/коммерческие предложения, воронки, дела,
  товарные позиции, пользовательские поля, реквизиты + банковские реквизиты,
  дубли, умные процессы, генератор документов, валюты, веб-формы, отслеживание,
  адреса, списки обзвона, триггеры автоматизации, история стадий
- **Задачи и проекты** — создание, выполнение, делегирование, чек-листы,
  комментарии, затраченное время, потоки, канбан-стадии, планировщик, зависимости,
  поля задач, группы/проекты + темы
- **Чаты и мессенджер** — создание чатов, участники, сообщения, реакции, поиск,
  счётчики, уведомления, недавние; открытые линии (конфиги, сессии, операторы);
  чат-боты v2 (регистрация, команды, файлы)
- **Файлы (Диск)** — загрузка/скачивание/перемещение, версии, шаринг, публичные
  ссылки, права, attached-объекты
- **Календарь** — события, разделы, встречи, ресурсы, доступность, настройки
- **HR и орг** — пользователи, отделы, учёт времени + time control + сетевые
  диапазоны + графики + записи, приём/увольнение/перевод
- **Бизнес** — списки + секции, почта (вкл. письмо→задача/событие/чат/CRM), отчёты,
  маркетинг, бизнес-процессы + CRUD роботов/активностей, телефония (звонки, SIP,
  voximplant: обратные/инфо-звонки, TTS, линии, статистика), события
- **Торговый каталог** — товары, разделы, цены, типы цен, единицы измерения, НДС,
  коэффициенты, правила округления, наценки, складские документы, свойства
  товаров, вариации (offers), SKU, услуги, остатки
